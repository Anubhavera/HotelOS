...
async function getDashboardStats(orgId: string) {
  ...
  // IST = UTC+5:30. Compute start of today in IST as a UTC timestamp
  const nowUTC = new Date();
  const nowIST = new Date(nowUTC.getTime() + 5.5 * 60 * 60 * 1000);
  const todayIST = `${nowIST.toISOString().split("T")[0]}T00:00:00`;
  const todayUTC = new Date(new Date(todayIST).getTime() - 5.5 * 60 * 60 * 1000).toISOString();
